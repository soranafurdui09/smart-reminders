...
export default function BottomNav({
  ...
  return (
    <nav className="mobile-bottom-nav fixed bottom-0 left-0 right-0 z-40 border-t border-border/40 bg-[color:var(--bg-base)]/90 backdrop-blur safe-bottom">
          ...
          return (
            <button
              ...
              className={`flex min-h-[44px] flex-1 flex-col items-center justify-center gap-1 rounded-2xl px-2 py-1 text-[11px] font-semibold transition ${
                tab.active ? 'text-[color:rgb(var(--accent))]' : 'text-textInv/70'
              }`}
            ...
            >
              <span className={`flex h-9 w-9 items-center justify-center rounded-2xl ${tab.active ? 'bg-[color:rgba(245,158,11,0.16)] text-[color:rgb(var(--accent))]' : 'bg-black/10 text-textInv/70'}`}>
