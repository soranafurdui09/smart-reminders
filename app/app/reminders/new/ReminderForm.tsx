  ...
  return (
    <form ref={formRef} action={action} className="space-y-8" onSubmit={handleFormSubmit}>
      ...
      <div className="space-y-2">
        <p className="text-sm font-medium text-muted">{copy.remindersNew.quickTemplatesLabel}</p>
        <div className="flex flex-wrap gap-2">
          {quickTemplates.map((template) => (
            <button
              key={template.id}
              type="button"
              className="inline-flex items-center rounded-full border border-borderSubtle bg-surface px-3 py-1 text-sm text-ink transition hover:bg-surfaceMuted"
      ...
      <section ref={mainCardRef} className="rounded-2xl border border-borderSubtle bg-surface p-6 shadow-soft">
          ...
          {(['ai', 'manual', 'medication'] as const).map((mode) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${
                createMode === mode
                  ? 'border-primary/40 bg-surface text-ink shadow-sm'
                  : 'border-transparent bg-surfaceMuted text-muted hover:bg-surface'
              }`}
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
                      ...
                      <button
                        ...
                        className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
                          voiceCreateMode === mode
                            ? 'border-primary/40 bg-surface text-ink'
                            : 'border-borderSubtle bg-surfaceMuted text-muted hover:bg-surface'
                        }`}
          ...
          <div
          className={`rounded-xl border border-borderSubtle bg-surface p-4 shadow-sm transition ${
